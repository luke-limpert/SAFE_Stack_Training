...
type Msg =
    ...
    | SetInput of string
    | AddTodo
    | AddedTodo of Result<Todo, AddTodoError>
...
let update msg model =
    match msg with
    | GetTodos ->
        { model with Todos = [] }, Cmd.OfAsync.either todosApi.getTodos () GotTodos UnhandledError
    | GotTodos todos ->
    ...
    | AddedTodo (Error error) ->
        { model with InputError = Some error }, Cmd.none
    | UnhandledError exn ->
        let errorDetails = $"{exn.GetPropagatedError()}. Please contact a system adminstrator."
        let alert = SimpleAlert(errorDetails).Title("An unhandled error occurred").Type(AlertType.Error)
        model, SweetAlert.Run alert
