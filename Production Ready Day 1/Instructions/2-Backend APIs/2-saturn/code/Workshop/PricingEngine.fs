...
let getPriceForRoute (request : EstimateRequest) =
    async {
        let! data =
            let url = sprintf """https://route.api.here.com/routing/7.2/calculateroute.json?app_id=1SsfVJTsVa5COwY7sNoE&app_code=q2QfgQchLOdW221Hvve6UQ&waypoint0=geo!%f,%f&waypoint1=geo!%f,%f&mode=fastest;car;traffic:disabled""" request.OLat request.OLong request.DLat request.DLong
