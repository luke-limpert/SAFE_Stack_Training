...
let getConnection () = new SqlConnection """Server=tcp:localhost,1433;Initial Catalog=sql-lab-facil;Persist Security Info=True;User ID=sa;Password=Str0ngPa55w0rd!;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=True;Connection Timeout=30;"""
                ...
                { TodoId = todo.TodoId
                  ...
                  Weekly =
                    if TaskRepeatState.isWeekly todo.RepeatState then
                        todo.RepeatState |> TaskRepeatState.getDay |> Day.Serialise |> Some
                    else
                        None
                  Custom =
            ...
            return
                ...
                |> Seq.map (fun (todoDto, user) ->
                    ...
                    let repeatState =
                        ...
                        | Some day,  _ -> day |> Day.Deserialise |> Weekly
